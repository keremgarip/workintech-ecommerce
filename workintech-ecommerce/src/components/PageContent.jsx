...
export default function PageContent() {
  return (
    <div className="w-full">
      {/* ShopCards */}
      <Container>
        <div className="flex flex-wrap justify-around px-2 py-20 gap-4">
          {shopCards.map((card) => (
            <div
              key={card.id}
              className="flex border border-[#ECECEC] p-4 justify-between w-full sm:w-[360px]"
            >
      ...
      <Container>
        <div className="px-2 py-12 flex flex-col lg:flex-row justify-center gap-7.5 items-stretch">
          ...
          >
            ...
            <Link to="/shop" className="text-sm font-bold text-[#23A6F0] mt-2">
              Shop Now →
            </Link>
          </div>

          <div className="flex-1">
            <div className="flex flex-wrap gap-2.5 items-center border-b-2 border-[#ECECEC]">
              <h3 className="font-bold text-base mr-2">Bestseller Products</h3>

              {bestSellerTabs.map((tab) => (
                <h6 key={tab} className="text-sm text-gray-600">
                  {tab}
                </h6>
              ))}

              <div className="flex py-3.5 px-5 ml-auto">
                <div className="border rounded-[34px] border-[#BDBDBD]">
                  <ChevronLeft className="w-5 h-5" />
                </div>
                <div className="border rounded-[34px] border-[#BDBDBD] ml-2">
          ...
          <section className="mt-2.5 flex flex-col md:flex-row">
            {benefits.map((benefit) => (
              <div key={benefit.id} className="flex p-6 gap-5">
                <h2 className="font-bold text-[40px] text-[#E74040]">
                  {benefit.id}.
                </h2>
                <div>
      ...
      <Container>
        <div className="px-2 py-12">
          <div className="flex items-center justify-between border-b-2 border-[#ECECEC] pb-3">
            <h3 className="font-bold text-2xl">PRODUCT PICKS</h3>
            <Link to="/shop" className="text-[#23A6F0] font-bold">
              See all →
            </Link>
          </div>

          <div className="flex flex-wrap gap-[30px] justify-center mt-6">
            {allProducts.slice(0, 6).map((product) => (
              <div
                key={product.id}
                className="flex flex-col items-center text-center gap-2 w-[240px]"
              >
      ...
      {/* Clients */}
      <Container>
        <div className="px-2 py-[50px] flex flex-wrap gap-10 items-center justify-center">
          {[
            "hooli",
            "lyft",
            "meta",
      ...
      <Container>
        <div className="px-2 py-28">
          <h6 className="font-bold text-sm text-[#23A6F0] text-center">
            Practice Advice
          </h6>
          <h2 className="font-bold text-[40px] text-center">Featured Posts</h2>

          <div className="flex flex-col md:flex-row gap-[30px] mt-8">
            {blogPosts.map((p) => (
